  ...
  <main class="page">
    <section class="hero">
      <div>
        ...
        <h1>劳务费税费换算工具</h1>
        <p class="desc">上传后台导出表格.xlsx，或在线输入少量数据，系统按新版0709规则生成客户台账。</p>
      </div>
    ...
    <section class="card tool-options">
      <label class="test-mode-toggle">
        ...
        <span><strong>测试模式</strong><em>开启后，结果预览会显示月累计维度、日累计维度、税后反推档位、个税档位和增值税规则。</em></span>
      </label>
    </section>
    <section class="card">
      <div class="tabs">
        <button :class="['tab', activeTab === 'upload' && 'active']" @click="activeTab = 'upload'">上传后台导出表</button>
        <button :class="['tab', activeTab === 'manual' && 'active']" @click="activeTab = 'manual'">在线输入</button>
      </div>
...
import { calculateManual, calculateUpload, downloadErrorTestTemplate, downloadLogicTestTemplate, downloadTemplate, exportLedger } from '../api/laborTaxApi'
...
async function runTask(task) {
  loading.value = true
  error.value = ''
  try { await task() } catch (err) { error.value = err.message || '操作失败' } finally { loading.value = false }
}
...
function handleUploadCalculate(file) { runTask(async () => applyResult(await calculateUpload(file))) }
function handleManualCalculate(rows) { runTask(async () => applyResult(await calculateManual(rows))) }
function handleExport() { runTask(async () => exportLedger(inputRows.value)) }
function clearResult() { resultRows.value = []; inputRows.value = []; summary.value = null; error.value = '' }
</script>

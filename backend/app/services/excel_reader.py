...
def find_header_row(ws) -> tuple[int, dict[int, str]]:
    """Find the customer backend-export header row."""
    ...
    raise ValueError("未识别到后台导出表头。请使用后台导出的Excel，需包含：会议日期、讲者姓名、讲者ID、劳务费（实付金额）。")


def _parse_event_date(value: Any) -> tuple[int, int, int]:
    if value in (None, ""):
    ...
    if isinstance(value, (int, float)):
        try:
            ...
            raise ValueError("会议日期格式不正确") from exc
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            if not 1 <= day <= 31:
                raise ValueError("会议日期日期必须在1-31之间")
...
def read_labor_rows(file_obj: BinaryIO) -> list[LaborInputRow]:
    ...
    for excel_row_idx in range(header_row + 1, ws.max_row + 1):
        ...
        if empty:
            continue
        try:
            year, month, day = _parse_event_date(event_date_value)
            raw["year"] = year
            raw["month"] = month
            raw["day"] = day
